...
def policy_improvement_soln(env, V, gamma=1):
    policy = np.zeros([env.observation_space, env.action_space]) / env.action_space
    for s in range(env.observation_space):
        q = q_from_v_soln(env, V, s, gamma)
        best_a = np.argwhere(q==np.max(q)).flatten()
        policy[s] = np.sum([np.eye(env.action_space)[i] for i in best_a], axis=0)/len(best_a)
    return policy

def policy_iteration_soln(env, gamma=1, theta=1e-8):
    policy = np.ones([env.observation_space, env.action_space]) / env.action_space
    while True:
...
env = FrozenLakeEnv()
random_policy = np.ones([env.observation_space, env.action_space]) / env.action_space
...
class Tests(unittest.TestCase):
    ...
    def q_from_v_check(self, q_from_v):
        ...
        soln = np.zeros([env.observation_space, env.action_space])
        to_check = np.zeros([env.observation_space, env.action_space])
        for s in range(env.observation_space):
